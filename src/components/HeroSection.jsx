...
export default function HeroSection() {
  return (
    <section className="px-5 md:px-20 bg-gray h-dvh overflow-hidden">
      <div className="flex flex-col md:flex-row">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="py-20 flex flex-col gap-8 w-full md:w-1/2 relative"
        >
          <span>
            <span className="bg-gray-two w-fit rounded-full text-neutral-300 px-4 py-1 text-sm">
              Available for work
            </span>
            <h1 className="text-4xl md:text-6xl font-manrope text-white max-w-full md:max-w-xl mt-6 break-words">
              Your trusted partner for quality home improvement
            </h1>
          </span>

          <p className="text-base md:text-lg leading-7 md:leading-8 text-neutral-300 font-manrope max-w-full md:max-w-lg break-words">
        ...
        <motion.div
          ...
          transition={{ duration: 0.6, delay: 0.2 }}
          viewport={{ once: true }}
          className="py-7 relative overflow-hidden h-fit flex w-full md:w-1/2 justify-end items-center"
        >
          <div className="relative w-full md:w-[580px]">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/40 rounded-xl"></div>
          </div>

          <div className="absolute top-5 left-5 md:top-14 md:left-15 text-white px-5 flex flex-col md:flex-row gap-5 z-10">
            {["home", "about", "ourWork", "contact"].map((item, idx) => (
              <a
                key={idx}
                href="#"
                className="text-sm md:text-lg capitalize hover:text-gray-300 transition"
              >
                {item}
              </a>
            ))}
